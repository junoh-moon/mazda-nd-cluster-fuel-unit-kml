...
Sibling codes on the same link: `B1252:09` (link-1 V_BUS on CMU 4C outside 4.8–5.2 V) and
`B1252:1D` (over-current ≥ 550 mA).

**What the link actually is (later-revision manuals, CX-8 2021 / Mazda3 BM mirrors):** the
CarPlay-era hub is connected to the CMU by **two independent USB links**, each GND / D− / V_BUS / D+:

| Link | CMU pins | Hub pins | Serves | DTC when silent | Code 61 field |
|---|---|---|---|---|---|
| 1 (original, the only link on the pre-CarPlay N243 hub) | 4A–4D | 2A–2D | secondary/charging port, AUX, SD slot | `B1252:87` "USB Port 1 – Missing Message" | HUB1 |
| 2 (added by the CarPlay retrofit cable) | 6A–6D | 3A–3D | CarPlay/Android Auto port | `B12B8:87` "USB Port 2 – Missing Message" | HUB2 |

So `B1252:87` means the hub's link-1 USB device no longer enumerates. "USB Port 1" in the DTC
text is the legacy link, not the phone-marked CarPlay port. On this car the pattern is exactly
"link 1 dead, link 2 alive": secondary port has no power and no data, HUB1 is Off, the CarPlay
port still runs Android Auto, and there is no `B1252:09`, so the CMU-side 5 V is fine and the
fault is inside the hub's link-1 half. AUX and the SD slot sit on the same link and are expected
to be dead too.

Diagnostic-assist code 61 ("Vehicle Signal") in the CMU hidden menu shows the live HUB1/HUB2
status, which is a direct way to see whether the CMU sees each half of the hub. On this car
code 61 showed HUB1: Off with every other signal On (2026-09-09).
...
|---|---|---|---|
| 1 | Hub PCB failure on the link-1 half – burnt/melted traces under the USB chips, or one port's circuit dead. "Charging OK / data dead / B1252:87" and "one port dead" cases were all fixed by a hub swap; a CX-5 KE delivered with one dead port got a warranty hub | Mazda3 BM owner with B1252:87: old hub had melted traces, new hub fixed it [D1]. CX-5 owner with lower port dead: dealer replaced hub as burnt [D2]. CX-5 KE: both ports dead, used OEM hub fixed it [D3]. Several Mazda3/CX-5 owners: one port on a genuine hub dies after years [D4] | High |
